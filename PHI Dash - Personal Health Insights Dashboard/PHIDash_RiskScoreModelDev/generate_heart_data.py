...
family_history = np.random.choice([0, 1], samples, p=[0.65, 0.35])

# Generating heart disease occurrence by calculating a risk score based on above factors and then increasing probability of occurrence for higher risk scores
heart_status = np.empty(samples, dtype=int)
...
for ind in range(samples):
    ...
    if riskscore > 3:
        heart_status[ind] = np.random.choice([0, 1], p=[0.2, 0.8])
    elif 2 < riskscore <= 3:
        heart_status[ind] = np.random.choice([0, 1], p=[0.5, 0.5])
    else:
        heart_status[ind] = np.random.choice([0, 1], p=[0.7, 0.3])
...
df.to_csv('generated_heart_data.csv', index=False)
